...
class SentEncoder(nn.Module):
    def __init__(self, embedding_dim=256, num_heads=6, hidden_dim=512, max_sent_length=100):
    ...
    def forward(self, x, _):
        ...
        # size should be (batch_size,number_of_heads, embedding_dim) if we use "han_sent_mha_attn_small" then--> embedding_dim*0.5
        # print(x.size())
        x = self.mean_pool(x.permute(0, 2, 1))
        # (x)shape= (batch_size, embedding_dim, 1) 
        # print(x.size())
        # applying linear classifier layer
        x = self.ffnn_clf(x.squeeze(2))#squeezed x shape= (batch_size, embedding_dim)
