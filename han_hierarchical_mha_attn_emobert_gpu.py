...
from han_word_attn_robert_emobert import WordAttn
from han_sent_enc import SentEncoder
...
class HierAttn(nn.Module):
    def __init__(self, word_hidden_size, sent_hidden_size, batch_size,num_classes, word2vec_path, max_sent_length, max_word_length, embd_size, model_name):
        ...
        self.sent_attn_layer    =   SentEncoder(embedding_dim=1024, num_heads=4, hidden_dim=512, max_sent_length=self.max_sent_length)
        
        self._init_hidden_state()
        
    def _init_hidden_state(self, last_batch_size=None,  device=None):
        if last_batch_size:
    ...
    #     self.word_hidden_state = self.word_hidden_state.to(device)
    #     self.sent_hidden_state = self.sent_hidden_state.to(device)
        # print("batch_size in model: ", self.batch_size)
        # print("hidden_state_shape: ", self.word_hidden_state.size())    
    def forward(self, input):
        
        device = input.device
        self._init_hidden_state(device=device)
        output_list=[]
        #print("@ hier attn")
        ...
        for i in input:
            ...
            #print("word_hidden_sate_from_word_attn",self.word_hidden_state.shape)
            output_list.append(output)
        
        output= torch.cat( output_list, 0)
        #print("in_to_sent_attn",output.shape)
        output, self.sent_hidden_state= self.sent_attn_layer(output.permute(1,0,2), self.sent_hidden_state)
