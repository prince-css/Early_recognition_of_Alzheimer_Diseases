...
class WordAttn(nn.Module):  # The module torch.nn contains different classess that help to
    ...
    def __init__(self, word2vec_path, model_name, embd_size=768, hidden_size=50):
        ...
        # in Parameter tensors "requires_grad = True" by default
        ...
        self.emobert = RobertaModel.from_pretrained("j-hartmann/emotion-english-distilroberta-base")
    ...
    def forward(self, input, hidden_state):
        ...
        # input_shape       = (N, n, L) or (batch_size=i, num_of_bert_features=n, max_word_length=k)
        ...
        #-----------------------------------------------------------------------------------------------------------------
        #=====================> Tokenize and impose BERT here <===========================
        wc_input_ids, wc_attention_masks=torch.split(input,[1,1],1) # splitting wc_encoding for getting those features to put into BERT
        sequence_output, pooled_output= self.bert(wc_input_ids.squeeze(1), wc_attention_masks.squeeze(1), return_dict= False)
        sequence_output, pooled_output2= self.emobert(wc_input_ids.squeeze(1), wc_attention_masks.squeeze(1), return_dict= False)
        
        #print(pooled_output2)
        #print(pooled_output2.size())
        #print("pooled_output_shape", pooled_output.shape)
        pooled_output=torch.unsqueeze(pooled_output, 0)
        pooled_output2=torch.unsqueeze(pooled_output2, 0)
        total_pooled_output=torch.cat((pooled_output,pooled_output2),2)
        #print(total_pooled_output.size())
        #print("pooled_output_shape_after_unsqueezing", pooled_output.shape)
        #print("sequence_output_shape", sequence_output.shape)
        #! x=sequence_output # shape(batch_size, max_word_length, embedding_dim) e.g.(16, 512, 768)
        #-----------------------------------------------------------------------------------------------------------------
        # input to GRU unit : : :
        ...
        #! x=x.permute(1,0,2)  # permuting for satisfying the GRU requierment when batch_first=False
                            # x shape= (L, N, H_in) or (max_word_length= k, batch_size= i, embedding_dim) e.g. (512, 16, 768)
        #print("x_shape after permuting= ",x.shape)                       
        ...
        return total_pooled_output, hidden_state
